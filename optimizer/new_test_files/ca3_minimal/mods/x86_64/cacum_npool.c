...
#define nrn_init _nrn_init__cacum_npool
#define _nrn_initial _nrn_initial__cacum_npool
#define nrn_cur _nrn_cur__cacum_npool
#define _nrn_current _nrn_current__cacum_npool
#define nrn_jacob _nrn_jacob__cacum_npool
#define nrn_state _nrn_state__cacum_npool
#define _net_receive _net_receive__cacum_npool 
#define integrate integrate__cacum_npool 
...
#define v _p[6]
#define _g _p[7]
#define _ion_ican	*_ppvar[0]._pval
#define _ion_cani	*_ppvar[1]._pval
#define _style_can	*((int*)_ppvar[2]._pvoid)
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_cacum_npool", _hoc_setdata,
 0, 0
};
 ...
 "depth_cacum_npool", "nm",
 "tau_cacum_npool", "ms",
 "cani0_cacum_npool", "mM",
 0,0
};
...
static void nrn_alloc(Prop* _prop) {
 ...
 nrn_promote(prop_ion, 3, 0);
 	_ppvar[0]._pval = &prop_ion->param[3]; /* ican */
 	_ppvar[1]._pval = &prop_ion->param[1]; /* cani */
 	_ppvar[2]._pvoid = (void*)(&(prop_ion->dparam[0]._i)); /* iontype for can */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 cacum_npool /home/tarluca/optimizer_20220509/ca3_minimal_for_pygmo/mods/cacum_npool.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dcani = - ican / depth / F / 2.0 * ( 1e7 ) + ( cani0 - cani ) / tau ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dcani = Dcani  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tau )) ;
  return 0;
}
 /*END CVODE*/
...
static void _thread_cleanup(Datum* _thread) {
 ...
 static void _update_ion_pointer(Datum* _ppvar) {
   nrn_update_ion_pointer(_can_sym, _ppvar, 0, 3);
   nrn_update_ion_pointer(_can_sym, _ppvar, 1, 1);
 }
...
for (_iml = 0; _iml < _cntml; ++_iml) {
 ...
 initmodel(_p, _ppvar, _thread, _nt);
  _ion_cani = cani;
  nrn_wrote_conc(_can_sym, (&(_ion_cani)) - 1, _style_can);
}
}
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(cani) - _p;  _dlist1[0] = &(Dcani) - _p;
 _slist2[0] = &(cani) - _p;
_first = 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/tarluca/optimizer_20220509/ca3_minimal_for_pygmo/mods/cacum_npool.mod";
static const char* nmodl_file_text = 
  "COMMENT\n"
  ...
  "NEURON {\n"
  "	SUFFIX cacum_npool\n"
  "	USEION can READ ican WRITE cani\n"
